...
class TestInstrumentFunction:
    """Test the instrument() function for patching Jinja environments."""

    def test_instrument_adds_jt_global(self) -> None:
        """Test that instrument() adds jt global to environment."""
        ...
        assert env.globals["jt"] is inst

    def test_instrument_test_mode_true(self) -> None:
        """Test instrument() with test_mode=True enables anchors/traces."""
        from jinja2 import Environment
        ...
        env = Environment()
        inst = instrument(env, test_mode=True)

        assert isinstance(inst, TestInstrumentation)

        # Test that anchors work
        template = env.from_string("{{ jt.anchor('test') }}Hello")
        result = template.render()
        assert "Hello" in result

    def test_instrument_test_mode_false(self) -> None:
        """Test instrument() with test_mode=False returns no-op instrumentation."""
        from jinja2 import Environment
        ...
        env = Environment()
        inst = instrument(env, test_mode=False)

        assert isinstance(inst, ProductionInstrumentation)

        # Test that anchors are no-ops
        template = env.from_string("{{ jt.anchor('test') }}Hello")
        result = template.render()
        assert result == "Hello"  # No anchor marker

    def test_instrument_custom_global_name(self) -> None:
        """Test instrument() with custom global_name."""
        from jinja2 import Environment

        from jinjatest import instrument

        env = Environment()
        instrument(env, global_name="instr")

        assert "instr" in env.globals
        assert "jt" not in env.globals

        template = env.from_string("{{ instr.anchor('test') }}Hello")
        result = template.render()
        assert "Hello" in result

    def test_instrument_with_existing_environment(self) -> None:
        """Test instrument() works with pre-configured environments."""
        from jinja2 import Environment

        from jinjatest import instrument

        env = Environment()
        env.globals["my_var"] = "existing"

        instrument(env)

        assert env.globals["my_var"] == "existing"  # Preserved
        assert "jt" in env.globals  # Added
